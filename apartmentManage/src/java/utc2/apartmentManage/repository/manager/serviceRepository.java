package java.utc2.apartmentManage.repository.manager;
...
public class serviceRepository {
    ...
    public List<Service> getPersonalServices() {
        List<Service> services = new ArrayList<>();
        String query = "SELECT * FROM services WHERE relevant = ?";
        try (Connection con = databaseConnect.getConnection();
             PreparedStatement pstmt = con.prepareStatement(query)) {
            
            pstmt.setString(1, "Cá nhân");
    ...
    public List<Service> getRegisterServices() {
        List<Service> services = new ArrayList<>();
        String query = "SELECT * FROM services WHERE relevant = ?";
        try (Connection con = databaseConnect.getConnection();
             PreparedStatement pstmt = con.prepareStatement(query)) {
    ...
    public boolean registerService(int accountID, int serviceID) {
        String sql = "INSERT INTO register_services (resident_id, service_id) VALUES (?, ?)";
        try (Connection conn = databaseConnect.getConnection();
    ...
    public boolean unregisterService(int accountID, int serviceID) {
        String sql = "DELETE FROM register_services WHERE resident_id = ? AND service_id = ?";
        try (Connection conn = databaseConnect.getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
